...
sensor_range = 4
motion_noise = np.array([0.015, 0.015, 0.005])
measure_noise = np.array([0.004, 0.004])
...
for idx in range(eval_time):
    ...
    log1 = erg_ctrl_sim1.start(report=True, debug=True, update=0, update_threshold=threshold)
    ...
    log2 = erg_ctrl_sim2.start(report=True, debug=True, update=1, update_threshold=threshold)
    erg_ctrl_sim2.animate()
    # erg_ctrl_sim2.new_animate3()
    ...
    # erg_ctrl_sim3.new_animate3(point_size=1, alpha=1, show_traj=True, title='Landmarks Distribution Test', rate=50)
