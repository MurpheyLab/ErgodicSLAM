...
for i in range(landmark_mean.shape[0]):
    distr = mvn.pdf(grid, landmark_mean[i], landmark_cov[i] * 20.)
...
# sim some trajectory
x_traj_1 = np.concatenate((np.linspace(2.5, 5, 15), np.linspace(5, 12.5, 15), np.linspace(12.5, 15, 10)))
y_traj_1 = np.concatenate((np.linspace(2.5, 12.5, 15), np.linspace(12.5, 10, 15), np.linspace(10, 2.5, 10)))
...
ax1.set_xlim(0, 20)
ax1.set_ylim(0, 20)
ax1.set_title('Simulation', fontsize=15, pad=10)
# ax1.scatter(robot_mean[0], robot_mean[1], color='red')
ax1.scatter(x_traj_1, y_traj_1, color='red')
for i in range(landmark_mean.shape[0]):
...
ax1.legend(['Robot Pose', 'Landmark'], fontsize=15, shadow=True)
...
ax2.axis('off')
ax2.set_title('Fisher Information', fontsize=15, pad=10)
...
ax3.axis('off')
ax3.set_title('Occupancy Grid Map', fontsize=15, pad=10)
ax3.set_aspect('equal')
for i in range(num_pts):
    for j in range(num_pts):
        cell = np.array([temp_grid[0][i][j], temp_grid[1][i][j]])
        # for lm in landmark_mean:
        for lm in np.stack((x_traj_1, y_traj_1)).T:
...
def mutual_info2(grid, og_vals, g):
    og = og_vals.copy()
    dist = (grid[0]-g[0])**2 + (grid[1]-g[1])**2
    dist = dist[0:num_pts, 0:num_pts]
    dist_flag = (dist < 16.).astype(int)
    obsv_flag = np.ones((num_pts, num_pts)) - og
    mi_flag = dist_flag * obsv_flag
    new_og = og + mi_flag
    return np.sum(entropy(new_og))

def mutual_info3(dist_flag, og_vals):
    og = og_vals.copy()

    ext_og = np.array([og.ravel() for _ in range(num_pts**2)])
    obsv_flag = -ext_og + 1.

    mi_flag = obsv_flag * dist_flag
    new_og = ext_og + mi_flag
    new_entropy = entropy(new_og)
    new_entropy = np.sum(new_entropy, axis=1).reshape(num_pts, num_pts)

    obsv_flag = (og > 0.5).astype(int)
    new_entropy *= obsv_flag
    return new_entropy

full_entropy = np.sum(entropy(og_vals))

ax4 = fig.add_subplot(154)
# ax4 = fig.add_subplot(gs01[1,1])
ax4.axis('off')
ax4.set_title('Mutual Information', fontsize=15, pad=10)
ax4.set_aspect('equal')
mi_vals = np.zeros((num_pts, num_pts))

mi_start_t = time.time()
for i in range(num_pts):
    for j in range(num_pts):
        if og_vals[i][j] == 1.:
            cell = np.array([temp_grid[0][i][j], temp_grid[1][i][j]])
            mi_vals[i][j] = mutual_info2(temp_grid, og_vals, cell)
print('mi time(2): ', time.time() - mi_start_t)
print('mi_vals.max: ', mi_vals.max(), 'mi_vals.min: ', mi_vals.min())

grid_x = temp_grid[0][0:num_pts,0:num_pts].ravel()
grid_y = temp_grid[1][0:num_pts,0:num_pts].ravel()
diff_x = grid_x - grid_x[:, np.newaxis]
diff_y = grid_y - grid_y[:, np.newaxis]
dist_xy = diff_x**2 + diff_y**2
dist_flag = (dist_xy < 16.).astype(int)

mi_start_t2 = time.time()
mi_vals = mutual_info3(dist_flag, og_vals)
print('mi time(3): ', time.time() - mi_start_t2)
...
for i in range(num_pts):
    for j in range(num_pts):
        if mi_vals[i][j] != 0.:
            ...
            mi_vals[i][j] = full_entropy - mi_vals[i][j]
...
ax5.axis('off')
ax5.set_title('Combined Target Distribution', fontsize=15, pad=10)
...
p4 = ax5.pcolormesh(temp_grid[0], temp_grid[1], full_vals, cmap=cmap)
