...
from rt_erg_lib.ekf_mpc_simulation2 import simulation_slam
...
landmarks1 = np.random.uniform(0.5, 19.5, size=(6, 2))
landmarks2 = np.random.uniform(0.5, 19.5, size=(4, 2))
landmarks = np.concatenate((landmarks1, landmarks2))

landmarks = np.random.uniform(0.5, size-0.5, size=(30, 2))

sensor_range = 4
motion_noise = np.array([0.05, 0.05, 0.01])
# motion_noise = np.zeros(3)
measure_noise = np.array([0.02, 0.02])
# measure_noise = np.zeros(2)
erg_ctrl_sim = simulation_slam(size, init_state, t_dist, modelTrue, ergCtrlTrue, envTrue, modelDR, ergCtrlDR, envDR, tf, landmarks, sensor_range, motion_noise, measure_noise, static_test=48)
