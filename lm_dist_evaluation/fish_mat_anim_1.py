...
sensor_range = 4
# motion_noise = np.array([0.3, 0.3, 0.1]) ** 2
motion_noise = np.array([0.15, 0.15, 0.1]) ** 2
# measure_noise = np.array([0.15, 0.15]) ** 2
measure_noise = np.array([0.1, 0.1]) ** 2

means = [np.array([18., 2.]), np.array([2., 18.])]
vars = [np.array([1, 1])**2, np.array([1, 1])**2]
t_dist = TargetDist(num_pts=50, means=means, vars=vars, size=size)

landmarks = np.load('/home/msun/Code/ErgodicBSP/lm_dist_evaluation/clustered_3.npy')
