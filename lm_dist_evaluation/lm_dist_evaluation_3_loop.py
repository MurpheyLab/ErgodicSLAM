...
init_state = np.array([18.0, 2.0, 0.0])
sensor_range = 4
motion_noise = np.array([0.25, 0.25, 0.1]) ** 2
measure_noise = np.array([0.15, 0.15]) ** 2

# means = [np.array([14.5, 5.5]), np.array([6.5, 15.5])]
means = [np.array([8., 13.]), np.array([11., 9.])]
vars = [np.array([1.7, 1.7])**2, np.array([1., 1.])**2]
t_dist = TargetDist(num_pts=50, means=means, vars=vars, size=size)

landmarks = np.load('/home/msun/Code/ErgodicBSP/lm_dist_evaluation/clustered_5.npy')


eval_time = 10
eval_logs = []
for idx in range(eval_time):
    print('evaluation round: {}'.format(idx))
    ...
    log1 = erg_ctrl_sim1.start(report=True, debug=False, update=0, update_threshold=5e-4)
    ...
    log2 = erg_ctrl_sim2.start(report=True, debug=False, update=1, update_threshold=5e-4)
    ...
    log3 = erg_ctrl_sim3.start(report=True, debug=False, update=2, update_threshold=5e-4)

    ###################################
    # evaluation
    ###################################

    eval_log = evaluation([log1, log2, log3], eval_id=idx, init_dist=copy(t_dist), landmarks=landmarks)
