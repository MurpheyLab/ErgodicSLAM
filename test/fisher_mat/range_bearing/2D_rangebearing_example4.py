...
if __name__ == '__main__':
    ...
    # preprocess landmark multi-gaussian values
    lm_cov_table = []
    for i in range(num_lm):
        lm = means[2+2*i+1:2+2*i+3]
        cov = raw_cov[2+2*i+1:2+2*i+3, 2+2*i+1:2+2*i+3]
    ...
    start_time = time()
    for i in range(grid.shape[0]):  # iterate every grid in search space
        x = grid[i,:]
        fisher_mat_val = 0
        ...
        for j in range(num_lm):     # iterate every "observed" landmark
            ...
            if lm[0] !=0 and lm[1] != 0: # observed
                # mat = sample_expectation(lambda lm1,lm2: fisher_mat(x, [lm1,lm2], mcov_inv), lm, cov, 100)
                mat = fisher_mat(x, lm, mcov_inv) * lm_cov_table[j] # multi_gaussian(lm, lm, cov)
