    ...
    for i in range(grid.shape[0]): # every grid in search space
        for j in range(num_landmark): # every (observed) landmark
            if init_table[j] == 1:
                ...
                var = varc[j]
                var = np.array([[0.04, 0], [0, 0.04]])
                var_inv = np.linalg.inv(var)

                dm = derivative_measure(alpha, mean)
                z = range_bearing(robot, mean)
                gamma = range_bearing(alpha, mean)
                diff = z - gamma

                sdot = np.dot(np.dot(diff.T, var_inv), dm)

                if gamma[0] <= 4:
                    vals[i] += np.linalg.det(np.outer(sdot, sdot) + np.eye(2))
                    # vals[i] += np.linalg.det(np.outer(sdot, sdot))
                else:
                    vals[i] += 0
            else:
                pass
