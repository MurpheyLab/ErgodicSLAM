...
class simulation():
    def __init__(self, size, init_state, model, erg_ctrl, env, tf, ped_data, space, goals=None):
        ...
        self.horizon = self.erg_ctrl.horizon
        self.env = env
        self.tf = tf
        self.model = model
        self.agent_sim = IntegratorSE2(size=self.size)
    ...
    def start(self):
        ...
        for t in tqdm(range(self.tf)):
            ...
            self.erg_ctrl.phik = convert_phi2phik(self.erg_ctrl.basis,
                                                  ...
                                                  self.grid)
            # ergodic control
            ctrl, u_seq = self.erg_ctrl(state, get_useq=True)
            self.log['agent_sim'].append(self.simulate_useq(state, u_seq))
    ...
    def simulate_useq(self, state, u_seq):
        self.agent_sim.reset(state)
        sim_states = []
        for u in u_seq:
            new_state = self.agent_sim.step(u)
            sim_states.append(new_state)
    ...
    def distance_field5(self, grid, state, space, threshold=0.1):
        start = time.time()
        grid_x = grid[:,0]
        grid_y = grid[:,1]
    ...
    def animate2(self, point_size=1, show_label=False, show_traj=True, save=None, rate=50):
        ...
        # points = ax.scatter([], [], s=point_size, c='r')
        points = ax.plot(xt[0,0], xt[0,1], 'ro')
        peds = ax.scatter([], [], s=point_size, c='b')

        sim_traj = ax.scatter([], [], s=point_size/4, c='y')
        ...
        def sub_animate(i):
            ...
            if(show_traj):
                ...
                # points.set_offsets(np.array([[xt[i, 0]], [xt[i, 1]]]).T)
                points[0].set_xdata(xt[i,0])
                points[0].set_ydata(xt[i,1])
            ...
            ax2.scatter(self.log['agent_sim'][i][:,0], self.log['agent_sim'][i][:,1], s=point_size/4, c='y')

            ret = [sim_traj, points[0], peds]
            return ret
