...
class simulation_slam():
    def __init__(self, size, init_state, t_dist, model_true, erg_ctrl_true, env_true, model_dr, erg_ctrl_dr, env_dr, tf,
        ...
        self.true_og_vals = np.ones(self.num_pts * self.num_pts) * 0.5
    ...
    def start(self, report=False, debug=False, update=1, update_threshold=1e-3, snapshot=0):
        ...
        cov = np.ones((self.nStates, self.nStates)) * 1e-09
        ...
        for t in tqdm(range(self.tf)):
            ...
            # update OG
            temp_grid = np.c_[self.grid[0][0:self.num_pts-1,0:self.num_pts-1].ravel(), self.grid[1][0:self.num_pts-1,0:self.num_pts-1].ravel()]
            for idx in range(len(temp_grid)):
                g = temp_grid[idx]
            ...
            if clip_idx_y < 0:
                clip_idx_y = 0
            clip_mean = np.array([self.grid[0][0][clip_idx_x], self.grid[0][0][clip_idx_y]])

            rv = mvn(mean=clip_mean[0:2], cov=cov[0:2, 0:2])
            prob = 0.5 * np.exp( -1. / (rv.pdf(self.grid2)+1e-09) ) ** 2.
            ...
            # evaluate and store ergodic metric
            xt_true = np.stack(self.log['trajectory_true'])
            path_true = xt_true[:i+1, self.model_true.explr_idx]
            ck_true = convert_traj2ck(self.erg_ctrl_dr.basis, path_true)
            erg_metric_true = self.erg_ctrl_dr.lamk * (ck_true - self.erg_ctrl_dr.phik)
            erg_metric_true = np.sum( erg_metric_true.reshape(1,-1) )
            self.log['metric_true'].append(erg_metric_true)

            xt_est = np.stack([m[0:3] for m in self.log['mean'] ])
            path_est = xt_est[:i+1, self.model_true.explr_idx]
            ck_est = convert_traj2ck(self.erg_ctrl_dr.basis, path_est)
            erg_metric_est = self.erg_ctrl_dr.lamk * (ck_est - self.erg_ctrl_dr.phik)
            erg_metric_est = np.sum( erg_metric_est.reshape(1,-1) )
            self.log['metric_est'].append(erg_metric_est)
    ...
    def evaluate(self):
        ...
        ax1 = fig.add_subplot(221)
        ax1.set_title('Uncertainty Plot')
        ax1.plot(np.arange(self.tf) * 0.1, self.log['uncertainty'])

        ax2 = fig.add_subplot(222)
        ax2.set_title('Estimation Error Plot')
        ax2.plot(np.arange(self.tf) * 0.1, self.log['error'])

        ax3 = fig.add_subplot(223)
        ax3.set_title('Ergodic Metric Plot')
        ax3.plot(np.arange(self.tf) * 0.1, self.log['metric_est'])
        ax3.plot(np.arange(self.tf) * 0.1, self.log['metric_true'])
        ax3.legend(['Est Traj', 'True Traj'])

        ax4 = fig.add_subplot(224)
        ax4.set_title('Area Coverage')
        ax4.plot(np.arange(self.tf) * 0.1, self.log['area_cov_est'])
        ax4.plot(np.arange(self.tf) * 0.1, self.log['area_cov_true'])
        ax4.legend(['Est Traj', 'True Traj'])
